   ...
   %----------------------------------------------------------------------%
      ...
      %output as "raw"
        ...
        function displaydata(filename)
            ...
            Datapath = strcat([filepath fs 'GPR' fs 'Data']);
            
            
             Savepath = strcat([filepath fs 'GPR' fs 'Raw_Data']);
...
%------for saving, call arrays A and t again------------------------------
        ...
        %       from integer to floating point
            ...
            filename.Amplitude= Aagc;
                ...
                Savepath = strcat([filepath fs 'GPR' fs 'AGC']);
...
%             ylabel('time (ns)'); title('original');
            ...
            filepath = pwd;
             ...
             Savepath = strcat([filepath fs 'GPR' fs 'Dewow']);
...
%         xlim([min(filename.Time) max(filename.Time)]);
        ...
        filename.Amplitude= Asec;
         ...
         Savepath = strcat([filepath fs 'GPR' fs 'SEC']);
...
%         imagesc(filename.Distance,filename.Time,filename.Amplitude); colormap(gray); hold on;
   ...
   %amount to show first recorded reflections as "true" time zero.
    ...
    function T0shifts_obj(filename)
...
%save(outfile,'A','x','t');
            ...
            filepath = pwd;
                ...
                Savepath = strcat([filepath fs 'GPR' fs 'Time_Zero']);
